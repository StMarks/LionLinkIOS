...
//Uses JSONDecoder() to parse the data into the accessible structs
    ...
    var weeklyEvents: [[Event]] {
        var Classes = [[Event]]()
        let unordered = parsedInfo.schedule
        let timeFormatter = DateFormatter()
        timeFormatter.dateFormat = "HH:mm"
        var day = dateFormatter.date(from: unordered[0].startTime)
        var i = 0
        var idnum = 1
...
//            }
                ...
                let event = Event(id:idnum, name: block.title, startTime: block.startTime, endTime: block.endTime, color: Color.green)
                Classes[i].append(event)
            }
        }
        return Classes
    }
